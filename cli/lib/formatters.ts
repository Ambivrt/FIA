...
export function colorByAgent(slug: string, text: string): string {
  // Stabil färg per agent baserat på slug
  const colors = [chalk.cyan, chalk.magenta, chalk.blue, chalk.yellow, chalk.green, chalk.red, chalk.white, chalk.gray];
  let hash = 0;
  for (let i = 0; i < slug.length; i++) {
    hash = (hash * 31 + slug.charCodeAt(i)) | 0;
  }
  const colorFn = colors[Math.abs(hash) % colors.length];
  return colorFn(text);
}
