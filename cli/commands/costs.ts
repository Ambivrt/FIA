...
export function registerCostsCommand(program: Command): void {
  program
    ...
    .action(async () => {
      ...
      const lines: string[] = [];
      lines.push(
        `  ${EARTH.stone("Kostnad MTD:")}    ${GRADIENT.orange.bold(`${data.cost_mtd_sek.toFixed(1)} kr`)} ${trendArrow} ${chalk.dim(trendPct + " vs förra mån")}`,
      );
      lines.push(
        `  ${EARTH.stone("Content (vecka):")} ${chalk.white.bold(String(data.content_this_week))} publicerade`,
      );
      lines.push(`  ${EARTH.stone("Godkännandegrad:")} ${chalk.white.bold(approvalPct)} ${chalk.dim("(30 dagar)")}`);
      lines.push(
        `  ${EARTH.stone("Väntande:")}` +
